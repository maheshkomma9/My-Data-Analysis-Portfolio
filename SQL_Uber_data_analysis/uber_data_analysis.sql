...
SELECT DISTINCT Vehicle_Type AS Vehicles 
FROM uber_data;
...
-- 6. Total bookings for each vehicle type
...
-- 8. Maximum and minimum ride distance for completed bookings
SELECT MAX(Ride_Distance) AS maximum_distance,
       MIN(Ride_Distance) AS minimum_distance 
FROM uber_data 
WHERE Booking_Status = 'Completed';

-- 9. Completed vs cancelled rides
...
-- 10. Average customer and driver ratings per vehicle type
SELECT Vehicle_Type,
...
-- 13. Total distance covered by each vehicle type
...
FROM uber_data
GROUP BY Customer_ID
HAVING cancellations >= 2;

-- 15. Booking share of each vehicle type
SELECT Vehicle_Type,
       COUNT(*) AS count,
...
-- 16. Success rate of each vehicle type
SELECT Vehicle_Type,
...
FROM uber_data
WHERE Booking_Status = 'Completed' AND Payment_Method IS NOT NULL
GROUP BY Payment_Method
...
-- 20. Monthly bookings and total revenue
...
HAVING SUM(Booking_Value) > (
    ...
    FROM (
        SELECT SUM(Booking_Value) AS cust_booking_value
        FROM uber_data
        GROUP BY Customer_ID
    ) AS avg_table
)
...
FROM (
    SELECT Vehicle_Type,
           Ride_Distance,
           AVG(Ride_Distance) OVER (PARTITION BY Vehicle_Type) AS avg_distance
    FROM uber_data
    WHERE Ride_Distance IS NOT NULL
) AS t
WHERE Ride_Distance > avg_distance;

-- 23. Booking value greater than average of that day
SELECT `Date`, Booking_Value, avg_booking
FROM (
    SELECT `Date`, Booking_Value,
           AVG(Booking_Value) OVER (PARTITION BY `Date`) AS avg_booking
    FROM uber_data
    WHERE Booking_Value IS NOT NULL
) AS a
WHERE Booking_Value > avg_booking;
...
-- 25. Moving average for a week
SELECT Date,
       AVG(Booking_Value) OVER (ORDER BY Date ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) AS moving_avg_booking
FROM uber_data;
